...
import { PlusCircle as PageIcon, PlusCircle, Loader2, AlertTriangle } from 'lucide-react';
...
import { z } from 'zod';
import { getSubjectsAction, createSubjectAction } from '@/app/actions/managementActions'; 
...
const subjectSchema = z.object({
  ...
  'Topics divided': z.string().optional(), 
});
    ...
    <div className="min-h-screen bg-background text-foreground flex flex-col">
      ...
      <main className="flex-1 container mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 md:space-y-10">
        <section className="flex flex-col md:flex-row items-center justify-between gap-4 md:gap-6">
          <div className="space-y-2">
            <div className="flex items-center space-x-3">
              <motion.div
                animate={{ rotate: [0, 10, -5, 0], scale: [1, 1.03, 1] }}
                transition={{ duration: 4, repeat: Infinity, repeatType: "mirror", ease: "easeInOut" }}
              >
                <PageIcon className="h-8 w-8 md:h-10 md:w-10 text-primary" />
              </motion.div>
              <h1 className="text-2xl md:text-4xl font-bold tracking-tight">Add Subject</h1>
            </div>
        ...
        <section className="space-y-6">
          <Card>
            <CardHeader>
              <CardTitle className="text-xl">New Subject Form</CardTitle>
        ...
        <section className="space-y-6">
          <h2 className="text-xl md:text-2xl font-semibold">Existing Subjects</h2>
           {isLoadingSubjects ? (
                      ...
                      {subjects.length === 0 ? (
                        ...
                        subjects.map((subject) => (
                          <TableRow key={subject.id}>
                            ...
                            <TableCell className="text-sm text-muted-foreground truncate max-w-xs md:max-w-sm lg:max-w-md">{subject.Topics || 'N/A'}</TableCell>
                            <TableCell className="text-sm text-muted-foreground truncate max-w-xs md:max-w-sm lg:max-w-md">{subject.Chapters || 'N/A'}</TableCell>
                            <TableCell className="text-sm text-muted-foreground truncate max-w-xs md:max-w-sm lg:max-w-md">{subject['Topics divided'] || 'N/A'}</TableCell>
                          </TableRow>
                        ))
                      )}
                    </TableBody>
        ...
        <section className="space-y-6">
          <h2 className="text-xl md:text-2xl font-semibold">Key Metrics</h2>
          <MetricsOverview />
        </section>
      </main>
    </div>
