...
const ApiTestChatInputSchema = z.object({
  ...
  apiKey: z.string().describe('The API key being discussed or "tested".'),
...
Describe what you *would* do or check if this were a real test environment.
For example, if the user says "Test the endpoint", you might respond with:
"Okay, if I were to test this '{{{apiConfigType}}}' API for '{{{apiUseCase}}}' with key '{{{apiKey}}}', I would typically start by sending a basic health check request (e.g., a GET request to a status endpoint if available). Then, I'd proceed with requests relevant to the use case, like fetching data, creating resources, etc., ensuring I check for expected status codes (200, 201, 400, 401, 403, 500), response times, and data integrity based on the API's specification."
