...
import { Copy, Edit2, Save, XCircle } from "lucide-react";
...
export function ViewNoteDialog({ isOpen, onOpenChange, note, onNoteUpdated }: ViewNoteDialogProps) {
  ...
  const handleSaveEdit = async () => {
    ...
    setIsSaving(true);
    try {
      const result = await editSubjectNoteAction(note.id, { Notes: editedNotes });
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleDialogClose}>
      <DialogContent className="sm:max-w-lg md:max-w-xl lg:max-w-2xl">
        <DialogHeader>
          <DialogTitle>
            {isEditing ? `Editing Note for: ${note.Subject} - ${note.Chapter}` : `${note.Subject} - ${note.Chapter}`}
          </DialogTitle>
          {!isEditing && (
            <DialogDescription>
